...
import PDFpage from './components/PDF-factory/PDFpage'
...
const MyApp = ()=> {
    ...
    const [formPages] =useState([
        ...
        {id:"basicInfo", title:"基本資料"},         
        {id:"attachment", title:"身分證&帳戶資料上傳"},         
        {id:"specification", title:"大誠業務人員行政規範"},         
        {id:"contract", title:"年度【業務承攬人員】合約書"},         
        {id:"pay60", title:"誠實險代扣授權書"},         
        {id:"exp", title:"同業經歷與佐證資料"},          
        {id:"premium", title:"免扣取補充保費聲明書"},         
        {id:"sign", title:"簽署板"},
        {id:"preview", title:"預覽"},
    ])
    ...
    */
    ...
    useEffect(()=>{
        ...
        //20220818 檢查是否有登錄
        webservice.asyncCheckLicense().then(res=>{            
            if (process.env.NODE_ENV == "development") {
                return  {
                        UID: "A220800045",
                        UserName: "呂雅惠/F125960334",
                        UserUnit: "總部",
                        Signatory: "0",
                        lifeLicenseID: "0093308333",
                        propLicenseID: "AB1F130542"
                    }
            }else{
                return webservice.asyncLogin()
            }
        }).then(res=>{
            dispatch({
                type:"SET_LOGIN_INFO",
            ...
            if(tempFileUid && tempFileUid !="NEW"){           
                ...
                webservice.asyncTempFileUid(tempFileUid).then((res)=>{
                    ...
                    dispatch({type:"SET_NSID", NSID:tempFileUid})   
                    res[1].map(item=>{
                        switch(item.ImgType){
                            case "大頭照":
    ...
    <div className={classes.root}>
        ...
        <Collapse style={{height:"100%", display:"flex",  justifyContent:"center", alignItems:"center"}} in={!open}>
            <CheckUser
                userChecker={ accessToken =>{
                    if(accessToken){
                        setOpen(true)
                    }else{
                    } 
        ...
        <Collapse in={open}>
                        ...
                        <div>   
                            ...
                            }
                            {
                                ...
                                <Button
                                    variant="contained"
                                    color="primary"
                                    //onClick={saveDataPdfFile}
                                    onClick={saveDataAndMakePDF}
                                    disabled={tempPDF==""?true:false}
                                    className={classes.button}
                                    style={{fontSize:24, borderRadius:12, backgroundColor:"#2dcc00"}}
                                >
                                確認上傳(上傳後無法修改資料)
                                </Button>
                            }
            ...
            <div style={{position:"fixed", padding:10, width:"100%",left:0, bottom:0, backgroundColor:"#fff", fontSize:18, borderTop:"1px solid #ccc", zIndex:"1"}}>
                <Button
                    variant="contained"
                    color="primary"
                    className={classes.button}
