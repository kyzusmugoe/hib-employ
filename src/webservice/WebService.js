class WebService {
    constructor() {
        ...
        this.WSLoginUrl = "../../Ashx/login.ashx";
        if (process.env.NODE_ENV == "development") {
            this.WSurl = "http://localhost:56400/ashx/NewSales.ashx";
            this.WSLoginUrl = "http://localhost:56400/ashx/login.ashx";
        }
    
        console.log(this.WSurl)
        console.log(this.WSLoginUrl)
    ...
    //20220818 檢查徵員者是否有登入在大誠
    asyncCheckLicense = async () => {
    ...
    checkLicense = ()=>{
        ...
        return new Promise((resolve, reject) => {
            fetch(this.WSurl, {
                ...
                console.log(result) 
                if (result !== undefined && result[0].hasLicence == "1") {
                    resolve(result)
                } else {
                    alert("您尚未在大誠登錄!無法使用此系統，謝謝。")
                    window.location = "../../html/login.html"
                }
            }).catch((error) => {
                console.error(error);
